"use client";

import * as React from "react";
import { useParams, useRouter } from "next/navigation";
import { ArrowLeft, ArrowRightLeft, Loader2, Printer } from "lucide-react";
import { PageHeader } from "@/components/layout/PageHeader";
import { Button } from "@/components/ui/Button";
import { Dropdown, DropdownTrigger, DropdownMenu, DropdownItem } from "@/components/ui/Dropdown";
import { QuotePreview } from "@/components/quotes/QuotePreview";
import { useToast } from "@/components/ui/Toast";
import { useAuth } from "@/context/AuthContext";
import { useOrganization } from "@/hooks/useOrganization";
import { quoteService } from "@/lib/services/quoteService";
import { clientService } from "@/lib/services/clientService";
import { DEFAULT_TAX_RATES } from "@/lib/constants/taxRates";
import type { Client, Quote, QuoteStatus } from "@/types";

const STATUS_OPTIONS: QuoteStatus[] = ["draft", "sent", "accepted", "declined", "expired"];

export default function QuoteDetailPage() {
  const params = useParams<{ id: string }>();
  const router = useRouter();
  const { toast } = useToast();
  const { profile } = useAuth();
  const { organization } = useOrganization();
  const organizationId = profile?.organizationId;
  const quoteId = params.id;

  const [quote, setQuote] = React.useState<Quote | null>(null);
  const [client, setClient] = React.useState<Client | null>(null);
  const [loading, setLoading] = React.useState(true);
  const [converting, setConverting] = React.useState(false);

  const load = React.useCallback(async () => {
    if (!organizationId) return;
    setLoading(true);
    const q = await quoteService.get(organizationId, quoteId);
    setQuote(q);
    if (q) {
      setClient(await clientService.get(organizationId, q.clientId));
    }
    setLoading(false);
  }, [organizationId, quoteId]);

  React.useEffect(() => {
    load();
  }, [load]);

  async function handleStatusChange(status: QuoteStatus) {
    if (!profile?.organizationId || !quote) return;
    await quoteService.markStatus(profile.organizationId, quote.id, status);
    toast({ variant: "success", title: `Marked as ${status}` });
    load();
  }

  async function handleConvert() {
    if (!profile?.organizationId || !quote) return;
    setConverting(true);
    try {
      const invoiceId = await quoteService.convertToInvoice(profile.organizationId, quote, DEFAULT_TAX_RATES);
      toast({ variant: "success", title: "Converted to invoice" });
      router.push(`/invoices/${invoiceId}`);
    } catch {
      toast({ variant: "error", title: "Couldn't convert quote", description: "Please try again." });
    } finally {
      setConverting(false);
    }
  }

  if (loading) {
    return (
      <div className="flex min-h-[40vh] items-center justify-center">
        <Loader2 className="h-6 w-6 animate-spin text-primary" />
      </div>
    );
  }

  if (!quote) {
    return <p className="text-muted-foreground">Quote not found.</p>;
  }

  return (
    <div>
      <PageHeader
        title={quote.number}
        description="Review, print or convert this quote."
        action={
          <div className="flex items-center gap-2 print:hidden">
            <Button variant="outline" size="sm" onClick={() => router.push("/quotes")} className="gap-2">
              <ArrowLeft className="h-4 w-4" /> Back
            </Button>
            <Button variant="outline" size="sm" onClick={() => window.print()} className="gap-2">
              <Printer className="h-4 w-4" /> Print / PDF
            </Button>
            {quote.status !== "converted" && (
              <Button size="sm" onClick={handleConvert} loading={converting} className="gap-2">
                <ArrowRightLeft className="h-4 w-4" /> Convert to invoice
              </Button>
            )}
            <Dropdown>
              <DropdownTrigger>
                <Button variant="outline" size="sm">
                  Update status
                </Button>
              </DropdownTrigger>
              <DropdownMenu>
                {STATUS_OPTIONS.map((status) => (
                  <DropdownItem key={status} onSelect={() => handleStatusChange(status)}>
                    Mark as {status}
                  </DropdownItem>
                ))}
              </DropdownMenu>
            </Dropdown>
          </div>
        }
      />
      <QuotePreview quote={quote} client={client} organization={organization} />
    </div>
  );
}
