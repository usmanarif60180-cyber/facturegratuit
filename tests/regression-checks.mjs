...
assert(index.includes('class="print-doc-summary"') && index.includes("Total TVA") && index.includes("Total TTC"), "Professional TVA summary is missing");
assert(index.includes("TVA non applicable, art. 293 B du CGI"), "French zero-TVA legal mention is missing");
assert(index.includes('documentType: "invoice"') && index.includes("calculateDocumentTotals(opts.items)"), "Invoice PDF tax calculations are not wired");
assert(index.includes('vat55: 5.5') && index.includes('vat21: 2.1'), "French reduced TVA rates are missing");
assert(index.includes('replace(/,/g, "")'), "Formatted document totals are parsed unsafely");
